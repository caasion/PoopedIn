...
const SEED_IMAGES = [
  "/uploads/seed-poop-1.svg",
  "/uploads/seed-poop-2.svg",
  "/uploads/seed-poop-3.svg",
  "/uploads/seed-poop-4.svg",
  "/uploads/seed-poop-5.svg",
];
...
async function main() {
  ...
  const users = await Promise.all([
    prisma.user.create({
      data: {
        name: "David Chen",
        avatarUrl: "https://api.dicebear.com/7.x/avataaars/svg?seed=david-chen",
    ...
    prisma.user.create({
      data: {
        name: "Marcus Webb",
        avatarUrl:
          "https://api.dicebear.com/7.x/avataaars/svg?seed=marcus-webb",
    ...
    prisma.user.create({
      data: {
        name: "Jennifer Park",
        avatarUrl:
          "https://api.dicebear.com/7.x/avataaars/svg?seed=jennifer-park",
    ...
    prisma.user.create({
      data: {
        name: "Tyler Brooks",
        avatarUrl:
          "https://api.dicebear.com/7.x/avataaars/svg?seed=tyler-brooks",
    ...
    prisma.user.create({
      data: {
        name: "Amanda Foster",
        avatarUrl:
          "https://api.dicebear.com/7.x/avataaars/svg?seed=amanda-foster",
  ...
  const tylerPosts = await Promise.all([
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(0), caption: "Day 1. Starting the journey. Every great career begins with a single step — or in this case, a single deposit. #Consistency", createdAt: daysAgo(13, 6) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(1), caption: "Day 2. The grind continues. Two days in and I can already feel the momentum building.", createdAt: daysAgo(12, 7) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(2), caption: "Day 3. Consistency is a muscle. You don't build it in a day, but you build it day by day. 🔥 #3DayStreak", createdAt: daysAgo(11, 7) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(3), caption: "Day 4. Still here. Still showing up. The market doesn't sleep and neither do I.", createdAt: daysAgo(10, 8) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(4), caption: "Day 5. Half a week strong. 🔥 If you're not posting daily, you're leaving gains on the table. #5DayStreak", createdAt: daysAgo(9, 7) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(0), caption: "Day 6. Discipline is choosing between what you want now and what you want most.", createdAt: daysAgo(8, 6) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(1), caption: "Day 7. One full week. 🔥 They said it couldn't be done. They were wrong. #7DayStreak #ThoughtLeadership", createdAt: daysAgo(7, 7) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(2), caption: "Day 8. Second week begins. Winners don't take days off.", createdAt: daysAgo(6, 8) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(3), caption: "Day 9. Your network is your net worth. Also your net girth. Showing up for both.", createdAt: daysAgo(5, 7) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(4), caption: "Day 10. Double digits. The algorithm rewards those who never stop delivering.", createdAt: daysAgo(4, 6) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(0), caption: "Day 11. At this point it's not a habit, it's an identity.", createdAt: daysAgo(3, 7) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(1), caption: "Day 12. I wake up at 4am every morning. The market doesn't sleep. Neither do I.", createdAt: daysAgo(2, 5) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(2), caption: "Day 13. One more day. Tomorrow I make history.", createdAt: daysAgo(1, 7) } }),
    prisma.post.create({ data: { userId: tyler.id, imageUrl: img(3), caption: "Day 14. 🔥🔥🔥 14 consecutive days. Nobody believed in me except me and my gastroenterologist. This is what winning looks like. #14DayStreak #Blessed", createdAt: daysAgo(0, 7) } }),
  ]);

  // David Chen — 7-day streak (days 0–6)
  const davidPosts = await Promise.all([
    prisma.post.create({ data: { userId: david.id, imageUrl: img(1), caption: "Grateful for this platform and all of you. Every morning I am reminded of what truly matters. 💩", createdAt: daysAgo(6, 8) } }),
    prisma.post.create({ data: { userId: david.id, imageUrl: img(2), caption: "Hot take: hustle culture is just culture. Change my mind.", createdAt: daysAgo(5, 9) } }),
    prisma.post.create({ data: { userId: david.id, imageUrl: img(3), caption: "LinkedIn is the only social network that rewards authenticity. Here is my authentic self.", createdAt: daysAgo(4, 7) } }),
    prisma.post.create({ data: { userId: david.id, imageUrl: img(4), caption: "Failure is just success that hasn't been leveraged yet. Keep going.", createdAt: daysAgo(3, 8) } }),
    prisma.post.create({ data: { userId: david.id, imageUrl: img(0), caption: "I wake up at 5am every morning and the first thing I do is deliver value. Today was no exception.", createdAt: daysAgo(2, 5) } }),
    prisma.post.create({ data: { userId: david.id, imageUrl: img(1), caption: "Your morning routine is a mirror of your professional ambitions. Mine is immaculate.", createdAt: daysAgo(1, 7) } }),
    prisma.post.create({ data: { userId: david.id, imageUrl: img(2), caption: "Excited to announce I've disrupted my own morning routine. Day 7 of showing up. #Consistency #ThoughtLeadership", createdAt: daysAgo(0, 8) } }),
  ]);

  // Sarah Mitchell — 5-day streak (days 0–4)
  const sarahPosts = await Promise.all([
    prisma.post.create({ data: { userId: sarah.id, imageUrl: img(2), caption: "Growth is not a destination. It's a direction. And mine is always forward.", createdAt: daysAgo(4, 9) } }),
    prisma.post.create({ data: { userId: sarah.id, imageUrl: img(3), caption: "The best time to start was yesterday. The second best time is right now. I chose right now.", createdAt: daysAgo(3, 8) } }),
    prisma.post.create({ data: { userId: sarah.id, imageUrl: img(4), caption: "Controversial opinion: if you're not measuring it, you're not managing it. I am measuring everything.", createdAt: daysAgo(2, 7) } }),
    prisma.post.create({ data: { userId: sarah.id, imageUrl: img(0), caption: "People ask me how I stay consistent. I tell them: I don't have a choice. This is who I am now.", createdAt: daysAgo(1, 8) } }),
    prisma.post.create({ data: { userId: sarah.id, imageUrl: img(1), caption: "5 days of uninterrupted value delivery. The market has spoken. 🔥 #5DayStreak #Growth", createdAt: daysAgo(0, 9) } }),
  ]);

  // Jennifer Park — 3-day streak (days 0–2)
  const jenniferPosts = await Promise.all([
    prisma.post.create({ data: { userId: jennifer.id, imageUrl: img(3), caption: "Day 1 of my new commitment to radical transparency. This is me, unfiltered.", createdAt: daysAgo(2, 10) } }),
    prisma.post.create({ data: { userId: jennifer.id, imageUrl: img(4), caption: "The algorithm rewards consistency. I am becoming the algorithm.", createdAt: daysAgo(1, 9) } }),
    prisma.post.create({ data: { userId: jennifer.id, imageUrl: img(0), caption: "3 days in. I've learned more about myself in 72 hours than in 3 years of executive coaching. #3DayStreak", createdAt: daysAgo(0, 10) } }),
  ]);

  // Amanda Foster — 2-day streak (days 0–1)
  const amandaPosts = await Promise.all([
    prisma.post.create({ data: { userId: amanda.id, imageUrl: img(4), caption: "My investors told me to build in public. This is as public as it gets.", createdAt: daysAgo(1, 11) } }),
    prisma.post.create({ data: { userId: amanda.id, imageUrl: img(0), caption: "Founders: stop waiting for the perfect moment. The perfect moment is when you go.", createdAt: daysAgo(0, 11) } }),
  ]);

  // Marcus Webb — CONSTIPATED (last post 4 days ago)
  const marcusPosts = await Promise.all([
    prisma.post.create({ data: { userId: marcus.id, imageUrl: img(1), caption: "I used to post every day. Then I got acquired and everything changed.", createdAt: daysAgo(30, 8) } }),
    prisma.post.create({ data: { userId: marcus.id, imageUrl: img(2), caption: "Back on my bullshit. New chapter, new metrics.", createdAt: daysAgo(15, 9) } }),
    prisma.post.create({ data: { userId: marcus.id, imageUrl: img(3), caption: "Shipping is a habit. I'm getting back into the habit.", createdAt: daysAgo(7, 8) } }),
    prisma.post.create({ data: { userId: marcus.id, imageUrl: img(4), caption: "Still here. Still building. The silence was intentional.", createdAt: daysAgo(4, 9) } }),
  ]);
